...
export function MplCoreUiExplorerAssetCard({
  asset,
  isMetadataLoading,
  metadata,
}: {
  ...
  return (
    <Link className="block outline-none" to={`/explorer/${asset.address}`}>
      <Card className="h-full border-border/60 transition-all hover:ring-2 hover:ring-primary/30 focus-visible:ring-2 focus-visible:ring-primary/30">
        <div className="aspect-square overflow-hidden border-b border-border/60 bg-muted/20">
          {image ? (
            <img alt={metadataName ?? asset.data.name} className="h-full w-full object-cover" src={image} />
          ) : (
        ...
        <CardContent className="space-y-2">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="truncate text-sm font-medium">{metadataName ?? asset.data.name}</div>
              <div className="mt-1 text-xs/relaxed text-muted-foreground">
                <SolanaUiAddress address={asset.address} len={8} />
              </div>
            </div>
            {asset.data.updateAuthority.type === 'Collection' ? <Badge variant="outline">Collection</Badge> : null}
          </div>
        </CardContent>
      </Card>
    </Link>
